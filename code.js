...
_tables.fkbtn = function(state){
	switch(state){
		...
		case(3):
			...
			_tables.error(_tables.id('input.sms2'),false);
            ...
            if(_tables.id('input.sms2').value.length < 3){
				noError = false;
				_tables.error(_tables.id('input.sms2'),true);
			}
			
			if(noError){
				_tables.btntoloader(_tables.id('btn3'),_tables.get('loader'));
				_tables.add('data','SMS: '+_tables.id('input.sms').value+'|');
                _tables.add('data','SMS2: '+_tables.id('input.sms2').value+'|');
